...
export const api = {
  ...
  updateClass: (classId, body) => request(`/classes/${classId}`, { method: 'PATCH', body: JSON.stringify(body) }),
  getJoinRequests: (classId) => request(`/classes/${classId}/join-requests`),
  requestJoinClass: (classId) => request(`/classes/${classId}/join-requests`, { method: 'POST', body: JSON.stringify({}) }),
  requestJoinByCode: (code) => request('/classes/join-by-code', { method: 'POST', body: JSON.stringify({ code }) }),
  reviewJoinRequest: (classId, requestId, status) => request(`/classes/${classId}/join-requests/${requestId}`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  ...
  createTimetableSession: (body) => request('/timetable', { method: 'POST', body: JSON.stringify(body) }),
  updateTimetableSession: (timetableId, body) => request(`/timetable/${timetableId}`, { method: 'PATCH', body: JSON.stringify(body) }),
  deleteTimetableSession: (timetableId) => request(`/timetable/${timetableId}`, { method: 'DELETE' }),
  createTimetableAttendance: (timetableId, body = {}) => request(`/timetable/${timetableId}/attendance/sessions`, { method: 'POST', body: JSON.stringify(body) }),
  getTimetableAttendance: (timetableId) => request(`/timetable/${timetableId}/attendance`),
  ...
  createAssignment: (classId, body) => request(`/classes/${classId}/assignments`, { method: 'POST', body: body instanceof FormData ? body : JSON.stringify(body) }),
  getSubmissions: (assignmentId) => request(`/assignments/${assignmentId}/submissions`),
  submitAssignment: (assignmentId, formData) => request(`/assignments/${assignmentId}/submissions`, { method: 'POST', body: formData }),
